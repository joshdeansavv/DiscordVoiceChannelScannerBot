...
A 24/7 Discord bot that streams audio to a voice channel with auto-reconnection.

Features:
- Automatic reconnection with exponential backoff
- Robust error handling for WebSocket issues
...
STREAM_URL = os.getenv("STREAM_URL", "https://audio.junctionnow.com:8000/radio.mp3")
...
if not GUILD_ID:
    print("❌ ERROR: DISCORD_GUILD_ID not found!")
    print("Please set your Discord guild (server) ID in one of these ways:")
    print("1. Create a .env file with: DISCORD_GUILD_ID=your_guild_id_here")
    print("2. Set environment variable: export DISCORD_GUILD_ID=your_guild_id_here")
    exit(1)

if not VOICE_CHANNEL_ID:
    print("❌ ERROR: DISCORD_VOICE_CHANNEL_ID not found!")
    print("Please set your Discord voice channel ID in one of these ways:")
    print("1. Create a .env file with: DISCORD_VOICE_CHANNEL_ID=your_channel_id_here")
    print("2. Set environment variable: export DISCORD_VOICE_CHANNEL_ID=your_channel_id_here")
    exit(1)
...
# Logging setup
log = logging.getLogger("discord_voice_bot")
logging.basicConfig(
    level=logging.INFO,
...
async def connect_and_play():
    ...
    try:
        ...
        if guild is None:
            log.error("Guild not found. Check DISCORD_GUILD_ID.")
            return False

        channel = guild.get_channel(VOICE_CHANNEL_ID)
        if channel is None or not isinstance(channel, discord.VoiceChannel):
            log.error("Voice channel not found or invalid. Check DISCORD_VOICE_CHANNEL_ID.")
            return False
        ...
        if not voice_client.is_playing():
            try:
                ...
                log.info("✅ Successfully started streaming audio")
                connection_attempts = 0  # Reset on success
                connection_backoff = 1
                return True
...
if __name__ == "__main__":
    try:
        print(f"Discord Voice Channel Scanner Bot")
